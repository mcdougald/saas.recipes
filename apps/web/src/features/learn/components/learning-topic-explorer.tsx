...
interface LearningTopicExplorerProps {
  ...
  favoriteTopicSlugs: string[];
}

/**
 * Interactive catalog for searching and filtering learning topics.
 */
export function LearningTopicExplorer({
  topics,
  favoriteTopicSlugs,
}: LearningTopicExplorerProps) {
        ...
        {filteredTopics.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filteredTopics.map((topic) => (
              <Card
                key={topic.slug}
                className="group relative h-full overflow-hidden border-border/70 bg-card dark:bg-muted/90 transition-all duration-300 hover:-translate-y-1 hover:border-primary/45 hover:shadow-lg focus-within:-translate-y-1 focus-within:border-primary/45 focus-within:shadow-lg"
              >
                <Link
